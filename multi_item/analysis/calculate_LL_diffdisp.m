function [LL,p_C_hat] = calculate_LL_diffdisp(x,data,model,logflag,nSamples)
%CALCULATE_LL_DIFFDISP calculates the LL under the assumption that the
%participant always knows their precision on the second display. Any
%ddifference between actual and assumed noise only takes place in
%the first display. 
