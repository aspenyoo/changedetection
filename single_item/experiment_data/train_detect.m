...
prefs = prefscode('Detect','Simult',subjid);
...
if sessionnum == 1;
    ...
    % ========== VM PRIOR OVER CHANGE ==========
    texty = screenCenter(2) - 150;
    Screen('DrawText',windowPtr,'The orientation from the ellipse to line will change in HALF OF THE TRIALS.',textx,texty,[255 255 255]); texty = texty + 3*dy;
    ...
    for iexamp = 1:nExamples;
        ...
        for isamp = 1:nSamples;
            ...
            deg2 = deg1+circ_vmrnd(0,prefs.vmprior,1)*180/pi;
            deg2
            [x,y] = lineCoord(prefs.lineLength, deg2);
...
Screen('Flip', windowPtr);
Practice_OrientationChangeDetect(subjid, sessionnum, nTrialsPerCond);
