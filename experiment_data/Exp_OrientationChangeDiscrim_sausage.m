...
for i = 1:length(prefs.reliabilityNum)
    ...
    for ideg = 1:numDegrees
        im = makeSausage(round(sqrt(prefs.ellipseArea)), ideg, s_length);
...
for i = 1:prefs.nTrials
    ...
    if (prefs.allStimInPres2)
        ...
        for j= 1:setsize
            srcrect = [0 0 squeeze(StimSizes(prefs.reliabilityNum == 0,pres1orientations(k(j)),:))'];
            destrect = CenterRectOnPoint(srcrect,xpositions(k(j)),ypositions(k(j)));
            Screen('drawtexture',windowPtr,StimPatches(prefs.reliabilityNum == 0,pres1orientations(j)),srcrect,destrect,0);
        ...
        srcrect = [0 0 squeeze(StimSizes(prefs.reliabilityNum == 0,pres1orientations(k(targetlocation)),:))'];
        destrect = CenterRectOnPoint(srcrect,xpositions(k(targetlocation)),ypositions(k(targetlocation)));
        Screen('drawtexture',windowPtr,StimPatches(prefs.reliabilityNum == 0,pres1orientations(targetlocation)),srcrect,destrect,0);
